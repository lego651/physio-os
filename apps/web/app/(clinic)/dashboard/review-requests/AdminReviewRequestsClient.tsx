...
export default function AdminReviewRequestsClient({ clinics, therapists, optOuts }: Props) {
  ...
  async function sendRow(row: ReviewRequest) {
    ...
    if (ch === 'missing') {
      showFlash(`${row.patient_name}: no contact info`)
      return
    }
    if (isRowOptedOut(row, clinicId, optOuts)) {
      showFlash(`${row.patient_name}: opted out`)
      return
    }
    if (!consentMap[row.id]) {
      showFlash('Check the consent box first')
      return
    }
  ...
  async function bulkSend() {
    ...
    if (sendable.length === 0) {
      showFlash('No rows ready to send (check consent checkboxes)')
      return
    }
    if (sendable.length > 10) {
      showFlash('Max 10 at a time. Deselect some rows.')
      return
    }
  ...
  const selectedConsented = rows.filter(
    (r) =>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-4 rounded-xl border bg-card p-4">
        <div className="flex items-center gap-2">
          ...
          <Select
            value={clinicId}
            onValueChange={(value) => {
              if (value) setClinicId(String(value))
            }}
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <Select
            value={globalChannel}
            onValueChange={(v) => {
              if (v) setGlobalChannel(v as typeof globalChannel)
            }}
          >
          ...
          <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
            ...
            <div className="flex flex-col gap-1">
              ...
              <Select
                value={draft.channel}
                onValueChange={(v) => {
                  if (v) setDraft((d) => ({ ...d, channel: v as typeof draft.channel }))
                }}
              >
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    <Input
                      ...
                      onChange={(e) =>
                        setEditPhone((prev) => ({ ...prev, [r.id]: e.target.value }))
                      }
                      placeholder="+1..."
                  ...
                  <td className="px-3 py-3">
                    <Input
                      ...
                      onChange={(e) =>
                        setEditEmail((prev) => ({ ...prev, [r.id]: e.target.value }))
                      }
                      placeholder="patient@..."
