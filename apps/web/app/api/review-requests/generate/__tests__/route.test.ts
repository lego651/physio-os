  ...
  from(table: string) {
    if (table === 'review_requests') {
      return {
        ...
        eq() { return this },
        async single() {
          return { data: { id: 'r1', therapist_name: 'Jimmy', service_type: 'massage', status: 'queued', clinics: { name: 'V-Health' } }, error: null }
        },
      }
    }
    return { select() { return this }, eq() { return this }, async maybeSingle() { return { data: null, error: null } }, async insert(r: any) { return { data: r, error: null } } }
  },
}) }))
