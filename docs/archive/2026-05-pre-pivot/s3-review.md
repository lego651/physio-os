...
The test file comments: _"Since detectKeyword is not exported, we replicate it here for unit testing."_ This means the test validates a **copy** of the function, not the actual implementation. If someone modifies the real `detectKeyword` in `route.ts`, the test will still pass with the old behavior.
