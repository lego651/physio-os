...
export function buildSystemPrompt(params: SystemPromptParams): string {
  ...
  const patientCondition = rawPatientCondition
    ? sanitizePromptValue(rawPatientCondition, 500)
    : undefined
  const practitionerName = rawPractitionerName
    ? sanitizePromptValue(rawPractitionerName, 100)
    : undefined
